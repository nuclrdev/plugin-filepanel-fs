...
/**
 * Final result of a copy run returned by {@link CopyService#execute}.
 *
 * <p>Individual per-file errors during an otherwise-successful run are reported
 * via {@link CopyProgressListener#onError} and do not change the outcome to
...
public enum CopyOutcome {
    ...
    /** The user interrupted the copy before it finished. */
    CANCELLED,

    /** An unrecoverable error prevented the copy from running at all. */
    FAILED
}
