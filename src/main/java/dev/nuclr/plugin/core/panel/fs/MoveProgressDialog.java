...
import dev.nuclr.plugin.core.panel.fs.move.MovePhase;
import dev.nuclr.plugin.core.panel.fs.move.MoveProgressSnapshot;

/**
 * Modeless progress dialog for a running move operation.
 *
 * <p>All public methods except {@link #setCancelAction} and {@link #show} must
 * be called from the Swing EDT.  The move background thread drives updates via
 * {@code SwingUtilities.invokeLater(() -> dialog.update(snap))}.
 *
 * <p>For same-filesystem moves the file bar snaps to 100% per file (atomic
 * rename); the total bar advances smoothly by accumulated file sizes.
 * For cross-filesystem moves both bars reflect live byte transfer progress.
 */
final class MoveProgressDialog {
    ...
    MoveProgressDialog(Component parent) {
        Window owner = SwingUtilities.getWindowAncestor(parent);
        dialog = new JDialog(owner, "Move", Dialog.ModalityType.MODELESS);
     ...
     */
    void update(MoveProgressSnapshot snap) {
        if (snap.phase() == MovePhase.SCANNING) {
        ...
        statsLabel.setText(
                snap.filesMoved() + " of " + snap.totalFiles() + " files  \u00b7  "
                + fmtBytes(snap.totalBytesTransferred()) + " of " + fmtBytes(snap.totalBytes()));
    }
     ...
     * double-clicks, then re-enabled when the action returns (handles the
     * case where the user says "No" to the cancel confirmation).
     */
    ...
    private void bindEscapeToCancel() {
        dialog.getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "cancelMove");
        dialog.getRootPane().getActionMap().put("cancelMove", new AbstractAction() {
